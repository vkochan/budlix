...
PKG_PYTHON_DISTUTILS_ENV = \
	PATH=$(BUILD_PATH) \
	PYTHONPATH="$(if $(PACKAGE_PYTHON3),$(PYTHON3_PATH),$(PYTHON_PATH))" \
	_python_prefix=$(INSTALL_DIR)/usr \
	_python_exec_prefix=$(INSTALL_DIR)/usr \
	$(BUILD_OPTS)
...
PKG_PYTHON_SETUPTOOLS_INSTALL_OPTS = \
	--prefix=/usr \
	--root=$(INSTALL_DIR)
	--executable=$(STAGING)/usr/bin/python \
...
$(2)_BASE_BUILD_OPTS   =
$(2)_BASE_INSTALL_OPTS = $$(PKG_PYTHON_DISTUTILS_INSTALL_OPTS)
# Setuptools
else ifeq ($$($(2)_SETUP_TYPE),setuptools)
$(2)_BASE_ENV         = $$(PKG_PYTHON_SETUPTOOLS_ENV)
$(2)_BASE_BUILD_TGT   = build
$(2)_BASE_BUILD_OPTS   =
$(2)_BASE_INSTALL_OPTS = $$(PKG_PYTHON_SETUPTOOLS_INSTALL_OPTS)
else
...
# the *_NEEDS_PYTHON variable.
#
# So:
# - for target packages, we always depend on the default python interpreter
#   (the one selected by the config);
# - for host packages:
#   - if *_NEEDS_PYTHON is not set, then we depend on use the default
#     interperter;
#   - otherwise, we depend on the one requested by *_NEEDS_PYTHON.
#
ifeq ($$($(2)_NEEDS_PYTHON),)
$(2)_DEPENDENCIES += $$(if $$(PACKAGE_PYTHON3),python3,python)
else
ifeq ($$($(2)_NEEDS_PYTHON),python2)
$(2)_DEPENDENCIES += python
else ifeq ($$($(2)_NEEDS_PYTHON),python3)
$(2)_DEPENDENCIES += python3
else
$$(error Incorrect value '$$($(2)_NEEDS_PYTHON)' for $(2)_NEEDS_PYTHON)
endif
endif # ($$($(2)_NEEDS_PYTHON),)
...
# <pkg>_NEEDS_PYTHON, and BR2_PACKAGE_PYTHON3 is used, then
...
ifeq ($$($(2)_NEEDS_PYTHON),python3)
$(2)_DEPENDENCIES += $$(if $$(filter python3-setuptools,$(1)),,python3-setuptools)
else ifeq ($$($(2)_NEEDS_PYTHON),python2)
$(2)_DEPENDENCIES += $$(if $$(filter python-setuptools,$(1)),,python-setuptools)
else ifeq ($$(PACKAGE_PYTHON3),y)
$(2)_DEPENDENCIES += $$(if $$(filter python3-setuptools,$(1)),,python3-setuptools)
else
$(2)_DEPENDENCIES += $$(if $$(filter python-setuptools,$(1)),,python-setuptools)
endif
...
# - for host packages:
#   - if *_NEEDS_PYTHON is not set, then we use use the default
#     interperter;
#   - otherwise, we use the one requested by *_NEEDS_PYTHON.
#
ifeq ($$($(2)_NEEDS_PYTHON),)
$(2)_PYTHON_INTERPRETER = $$(STAGING_DIR)/usr/bin/python
else
$(2)_PYTHON_INTERPRETER = $$(STAGING_DIR)/usr/bin/$$($(2)_NEEDS_PYTHON)
endif
