...
PERL_ARCHNAME = $(shell uname -m)-linux
PERL_RUN = PERL5LIB= PERL_USE_UNSAFE_INC=1 $(STAGING_DIR)/usr/bin/perl
...
define $(2)_CONFIGURE_CMDS
	cd $$($$(PKG)_SRCDIR) && if [ -f Build.PL ] && [ $$($(2)_PREFER_INSTALLER) != "EUMM" ] ; then \
		...
		$$(PERL_RUN) Build.PL \
			--config ar="$$(BUILD_AR)" \
			--config full_ar="$$(BUILD_AR)" \
			--config cc="$$(BUILD_CC)" \
			--config ccflags="$$(BUILD_CFLAGS)" \
			--config optimize=" " \
			--config ld="$$(BUILD_CC)" \
			--config lddlflags="-shared $$(BUILD_LDFLAGS)" \
			--config ldflags="$$(BUILD_LDFLAGS)" \
		...
		$$(PERL_RUN) Makefile.PL \
			...
			OPTIMIZE=" " \
			LD="$$(BUILD_CC)" \
			LDDLFLAGS="-shared $$(BUILD_LDFLAGS)" \
...
define $(2)_INSTALL_CMDS
	cd $$($$(PKG)_SRCDIR) && if [ -f Build.PL ] && [ $$($(2)_PREFER_INSTALLER) != "EUMM" ] ; then \
		$$(PERL_RUN) Build $$($(2)_INSTALL_OPTS) install; \
	else \
		$$(MAKE1) $$($(2)_INSTALL_OPTS) pure_install; \
	fi
endef
endif
